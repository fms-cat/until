  ...
  add( paths ) {
    ...
    for ( let name in paths ) {
      ...
      if ( path.framebuffer ) {
        ...
        if ( path.swapbuffer ) {
          if ( path.drawbuffers ) {
            path.swapbuffer = it.glCat.createDrawBuffers( path.width, path.height, path.drawbuffers );
          } else if ( path.float ) {
            path.swapbuffer = it.glCat.createFloatFramebuffer( path.width, path.height );
          } else {
            path.swapbuffer = it.glCat.createFramebuffer( path.width, path.height );
          }
        }
